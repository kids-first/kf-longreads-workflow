...
inputs:
  input_unaligned_bam: {type: 'File', secondaryFiles: [{pattern: '.bai', required: true}], doc: "Unaligned BAM file and index containing
  ...
  biospecimen_name: {type: 'string?', doc: "String name of the biospecimen. Providing this value will override the SM value provided
      in the @RG line of the input_unaligned_bam header."}
  sentieon_license: {type: 'string?', doc: "License server host and port for Sentieon tools.", default: "10.5.64.221:8990"}
  sentieon_dnascope_model: {type: 'File', doc: "Sentieon DNAscope model bundle.", "sbg:suggestedValue": {class: File, path: 684987b02b97f154cfc69c46,
      name: DNAscopeONT2.1.bundle}, "sbg:fileTypes": "BUNDLE"}
  minimap2_preset:
    type:
  ...
  nanocaller_regions: {type: 'string[]?', doc: "A space/whitespace separated list of regions specified as 'CONTIG_NAME' or 'CONTIG_NAME:START-END'.
      If you want to use 'CONTIG_NAME:START-END' format then specify both start and end coordinates. For example: chr3 chr6:28000000-35000000
      chr22."}
  nanocaller_include_bed: {type: 'File?', doc: "A BED file specifying regions for variant calling."}
  nanocaller_exclude_bed: {type: 'File?', doc: "A BED file specifying regions to exclude from variant calling."}
  nanocaller_preset:
    type:
      type: enum
  ...
  nanocaller_neighbor_threshold: {type: 'string?', doc: "SNP neighboring site thresholds with lower and upper bounds seperated by
      comma, for Nanopore reads '0.4,0.6' is recommended, for PacBio CCS anc CLR reads '0.3,0.7' and '0.3,0.6' are recommended respectively"}
  ...
  nanocaller_win_size: {type: 'int?', doc: "Size of the sliding window in which the number of indels is counted to determine indel
      candidate site.  Only indels longer than 2bp are counted in this window. Larger window size can increase recall, but use a maximum
      of 50 only"}
  ...
  nanocaller_maxcov: {type: 'int?', doc: "Maximum coverage of reads to use. If sequencing depth at a candidate site exceeds maxcov
      then reads are downsampled."}
  sex:
    type:
    - 'null'
    - type: enum
  ...
  cutesv_diff_ratio_merging_DEL: {type: 'float?', default: 0.3, doc: "Do not merge breakpoints with basepair identity more than the
      ratio of default for deletion."}
  sniffles_tandem_repeats_input_bed: {type: 'File?', doc: "Sniffles input .bed file containing tandem repeat annotations for the input
      reference genome. Providing this file can improve Sniffles call accuracy."}
  sniffles_non_germline: {type: 'boolean?', doc: "Request that Sniffles call non-germline SVs (rare, somatic or mosaic SVs)."}
...
outputs:
  ...
  nanocaller_small_variants: {type: 'File', secondaryFiles: [{pattern: '.tbi', required: true}], outputSource: nanocaller_merge_final/merged_vcf,
    doc: "VCF.GZ file and index containing NanoCaller-gerated small variant calls."}
  longreadsum_bam_metrics: {type: 'File', outputSource: tar_longreadsum_dir/output, doc: "TAR.GZ file containing longreadsum-generated
      metrics."}
  cutesv_structural_variants: {type: 'File', secondaryFiles: [{pattern: '.tbi', required: true}], outputSource: bgzip_tabix_index_cutesv_vcf/output,
    doc: "VCF.GZ file and index containing cutesv-generated SV calls."}
